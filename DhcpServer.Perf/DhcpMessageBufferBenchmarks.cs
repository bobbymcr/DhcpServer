...
namespace DhcpServer.Perf
{
    ...
    public class DhcpMessageBufferBenchmarks
    {
        ...
        {
            var option1 = this.buffer.WriteNextOption(DhcpOptionTag.DhcpMsgType, 1);
            option1.Data[0] = (byte)DhcpMessageType.Discover;
            var option2 = this.buffer.WriteNextOption(DhcpOptionTag.ClientId, 7);
            option2.Data[0] = (byte)DhcpHardwareAddressType.Ethernet10Mb;
            new MacAddress(0x000B8201FC42).WriteTo(option2.Data.Slice(1));
            var option3 = this.buffer.WriteNextOption(DhcpOptionTag.AddressRequest, 4);
            default(IPAddressV4).WriteTo(option3.Data);
            var option4 = this.buffer.WriteNextOption(DhcpOptionTag.ParameterList, 4);
